...
The source plugin adds two workflow skills and one local MCP server.
It uses the installed `fantasy-football-manager` command.
It does not add a provider login or a live execution adapter.
...
Check that the Codex process can find the command on `PATH`.
...
Ask Codex to call `get_capabilities`, then run the synthetic draft workflow.

The default personal marketplace is discovered implicitly.
No public marketplace entry is created by this process.
Use either this plugin or a direct MCP connection to this server to avoid duplicate tool sets.

## Inspect the source

- [Plugin manifest](../plugins/fantasy-football-manager/.codex-plugin/plugin.json)
- [MCP command](../plugins/fantasy-football-manager/.mcp.json)
...
The MCP file uses a relative-free command name. It contains no user path, token, or account identifier.
